# здесь функции блока request и для их работы в какой-то мере прописан функционал интерфейса
...
# печатает список сотров с полными данными - всю базу последовательно по каждому сотру - карточки сотрудника по строкам
...
# преобразуем строку выбора полей в список - техническая внутренняя ф-ция, но должна участвовать в main 
# для проверки ввода при определенном выборе пользователя
...
# если через GUI - дать возможность выбора
section = input('Выберите/Введите поле для поиска: ')
...
# при получении числа везде использовать отдельную ф-цию его проверки
...
if section in fields_int:
    ...
    if var == '1':
        el = int(input('Введите интересующее вас значение: '))
        print_for_key_value(staff, section, el)
    elif var == '2':
        el = []
        start = int(input('Введите значение для поиска ОТ: '))
        end = int(input('Введите значение для поиска ДО: '))
        el.append(start)
        el.append(end)
        print_for_key_value(staff, section, el)
    else:
...
# добавим работу с датой - другие варианты запроса
    ...
    if var == '1':
        ...
        el = int(input('Введите значение года: '))
        # по хорошему сюда поставить текущий год через ф-цию
        if 1950 < el <= int(date.today().year):
            el = (None, el)
            print_for_key_value(staff, section, el)
        else:
            # фикс лог
        ...
        el1 = int(input('Введите значение месяца: '))
        if 0 < el1 <= 12:
            el = (el1, None)
        else:
            # фикс лог
        ...
        el2 = int(input('Введите значение года: '))     
        if 1950 < el2 <= int(date.today().year):
            el = (el1, el2)
        else:
            # фикс лог
            print('Некорректное значение')
...
# list_items_for_key(staff,key) #она по сути не нужна? - это просто ограниченный список ФИО и ТН по заданному полю
...
str_fields = input(
    'Перечислите через запятую интересующие вас поля для выборки дынных: ')
...
print_list_worker(staff)
person = input(
    'Выберите/введите сотрудника, по которому нужно найти информацию: \n')
print_fields_for_choice()
str_fields = input(
    'Перечислите через запятую интересующие вас поля для выборки дынных: ')
# проверяем, если строка полей корректна и такой сотрудник существует
li_select = str_to_li_fields(str_fields)
if li_select and person in staff:
    print_select_fields(staff, li_select, person)
    # передается база, список полей для вывода и выбранный сотрудник
else:
    # здесь вызываем запись лога ошибки и выводим на экран
    print('Некорректный ввод')
