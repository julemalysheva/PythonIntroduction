...
def print_for_key_value(dict, key, item):
    ...
    for k in dict:
        if key in fields_int and isinstance(item,list):
            if item[0] <= dict[k][key] <= item[1]: #если значение этого ключа попадает в диапазон   
                print(f'ФИО: {k} \tТН: {dict[k]["ТН"]} \t{str(key)}: {dict[k][key]}')
                count_find+=1
...
# сюда еще добавить работу с датой
...
print_list_worker(staff)
person = input('Выберите/введите сотрудника, по которому нужно найти информацию: \n')
print_fields_for_choice()
str_fields = input('Перечислите через запятую интересующие вас поля для выборки дынных: ')
